...
namespace Aviasales
{
    public class Dispatcher
    {
            ...
            for (int i = 0; i < requests.Count(); i++) 
            {
                if (!requests[i].GetStatus())
                {
                    ...
                    else
                    {
                        ...
                        else if (requests[i] is TakeoffRequest && requests[i].GetAirRoute().ArriveTime > currentTime)
                        {
                            requests[i].Defer();
                        }
                    }
        ...
        private Runway FindRunwayForTakeoff() // Метод поиска полосы для взлёта
        {
            ...
            // Находим полосу с наибольшим соотношением посадок к сумме посадок и взлетов
